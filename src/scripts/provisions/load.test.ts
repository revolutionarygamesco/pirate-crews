...
import { createProvisionTypes } from './types.ts'
...
describe('loadProvisions', () => {
  ...
  it('returns the registered provision types', () => {
    const data = createProvisionTypes({ white: { label: 'Ketracel-white', unit: 'dose', units: 'doses', consumption: 1 } })
  ...
  it('falls back to default if it can’t get registered data', () => {
    ...
    const actual = loadProvisions()
    expect(actual).toEqual(createProvisionTypes())
  })
