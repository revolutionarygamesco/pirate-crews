  ...
  describe('Instance methods', () => {
    describe('getInstances', () => {
      it('returns the watch instances for the day of a given timestamp (in seconds)', () => {
        const set = new WatchSet()
        const timestamp = ((3 * 24) + 21) * SECONDS_PER_HOUR // 21 hours into day 3
        ...
        expect(actual[0].team).toBe('larboard')
        expect(actual[6].end).toBe(expectedCycleStart + (24 * SECONDS_PER_HOUR))
        expect(actual[6].team).toBe('larboard')
      })
    })
    ...
    describe('getCurrent', () => {
      it('returns the current watch instance', () => {
        ...
        const { name, duration, start, end, team } = set.getCurrent(timestamp)
